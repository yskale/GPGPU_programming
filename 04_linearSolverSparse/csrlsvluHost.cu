// nvcc -arch=sm_70 -lcublas -lcusolver -lcusparse csrlsvluHost.cu
// solve Ax=b, with LU or PLU factorization, only support host execution
...
int main()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        __TIME_BEGIN
#ifdef DOUBLE_FP_CASE
        cusolverSpDcsrlsvluHost(cusolverH, csrA_h.nA, csrA_h.numA, descrA, csrA_h.csrValA, csrA_h.csrRowPtrA, csrA_h.csrColIndA, vecb_h, 0.0, 0, resx_h, &singularity);
#else
        cusolverSpScsrlsvluHost(cusolverH, csrA_h.nA, csrA_h.numA, descrA, csrA_h.csrValA, csrA_h.csrRowPtrA, csrA_h.csrColIndA, vecb_h, 0.0, 0, resx_h, &singularity);
#endif
        __TIME_END
        std::cout << "No. " << i << " run, CPU (csrlsvlu only support Host) calculation time = " << elapsedTime << "ms\n";
    }
